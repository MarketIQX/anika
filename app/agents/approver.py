...
def _save_as_voice_example(draft_row: dict[str, Any], *, decided_by: str) -> int | None:
    ...
    Thin wrapper over voice_harvester.harvest_voice_example — single source
    of truth for voice_example creation across all pathways (this approval
    flow, the outbound harvester, and any future manual-upload path).

    Called from approver.approve() when the draft has parent_draft_id — meaning
    the partner edited at least once before approving. The approved body is the
    gold-standard voice for this service line.
    """
    from app.cognitive.voice_harvester import harvest_voice_example

    return harvest_voice_example(
        sent_body=draft_row.get("body") or "",
        service_line=(draft_row.get("likely_service_line") or "").strip() or None,
        created_by=decided_by,
        source="edit_approval",
        source_draft_id=draft_row.get("id"),
    )
