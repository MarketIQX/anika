"""Humility layer — articulates what Anika noticed and what she needs.
...
class UnknownArticulation(BaseModel):
    ...
    best_guess_purpose: Literal[
        "voice_example", "classifier_example", "document_type",
        "question_template", "workflow_rule", "firm_fact",
        "firm_policy", "reference_material",
    ] = Field(
        description="Her best guess among the 8 purposes. MUST be one of the 8 literals. If content needs a new category, pick the closest literal and set suggested_custom_label."
    )
